...
export function resolveActualKeys(
  ...
  // Single ASCII letter keypress (e.g. 'd', 'a', 'e', 'o', 'w', etc.)
  if (actualKey.length === 1 && /^[a-zA-Z]$/.test(actualKey)) {
    return [actualKey];
  }

  // At the start of a token (when no previous keys have been typed yet),
